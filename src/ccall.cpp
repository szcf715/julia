    ...
    if (jvinfo.isboxed) {
        if (!jl_is_abstracttype(ety)) {
            if (jl_is_mutable_datatype(ety)) {
                ...
                return ctx.builder.CreatePtrToInt(emit_pointer_from_objref(ctx, data_pointer(ctx, jvinfo)), to);
            }
        ...
        *needStackRestore = true;
        ...
        Value *p1 = ctx.builder.CreatePtrToInt(emit_pointer_from_objref(ctx, data_pointer(ctx, jvinfo)), to);
...
#define is_libjulia_func(name) _is_libjulia_func((uintptr_t)&(name), #name)
    ...
    if (is_libjulia_func(jl_array_ptr)) {
        ...
        Value *strp = ctx.builder.CreateConstGEP1_32(T_int8, emit_bitcast(ctx, obj, T_pint8), sizeof(void*));
        strp = ctx.builder.CreatePtrToInt(strp, lrt);
        JL_GC_POP();
