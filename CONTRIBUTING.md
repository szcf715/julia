...
If you install the [Revise](https://github.com/timholy/Revise.jl)
package, you can make extensive changes to Base without needing to
rebuild each time.

Here is the basic procedure:

1. If you are planning changes to any types or macros, make those
   changes, commit them, and build julia using `make`. You do not have
   to add any functionality based on those types, just the type
   definitions themselves. (This is necessary because `Revise` cannot
   handle changes to type definitions or macros.)
...
Should you for some reason need to quit your REPL session and restart
it before finishing your changes, you can do the following (from
somewhere in the `julia` directory):
...
shell> git stash pop  # restore any in-progress changes
```
